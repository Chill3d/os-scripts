#!/bin/bash
#-Metadata----------------------------------------------------#
#  Filename: install.sh             (Update: 09-04-2018)      #
#-Info--------------------------------------------------------#
...
################################################################################
wget -qO install.sh https://raw.githubusercontent.com/Chill3d/os-scripts/master/install.sh \
  && bash install.sh
################################################################################
fi
#-Defaults-------------------------------------------------------------#
...
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}kali-linux-large${RESET} meta-package"
...
apt -y -qq install kali-linux-large \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
...
#timeout 300 curl --progress-bar -k -L -f "https://github.com/notable/notable/releases/download/v1.8.4/notable_1.8.4_amd64.deb" > /tmp/notable.deb \
...
##### Install Keepass
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}keepass${RESET} ~ Password Manager"
apt -y -qq install keepass2 \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
...
##### Install eyewitness
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}EyeWitness${RESET} ~ Web screenshot on network"
apt -y -qq install eyewitness \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
...
echo -e " ${YELLOW}[i]${RESET} + ${BOLD}Create user and launch customize.sh"
